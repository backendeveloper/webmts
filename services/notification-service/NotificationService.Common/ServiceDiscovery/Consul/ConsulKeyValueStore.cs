...
namespace NotificationService.Common.ServiceDiscovery.Consul;
...
public class ConsulKeyValueStore : IKeyValueStore
{
    ...
    public async Task<IDictionary<string, string>> GetAllValuesAsync(string prefix)
    {
        ...
        try
        {
            ...
            if (result.Count == 0 && _configuration != null)
            {
                ...
                if (section != null)
                {
                    _logger.LogInformation("No values found in Consul for prefix {Prefix}, checking configuration",
                        prefix);
    ...
    public async Task<bool> SyncConfigurationSectionToConsulAsync(string sectionName, string consulPrefix)
    {
        ...
        try
        {
            ...
            if (initialized == "true")
            {
                _logger.LogInformation("Configuration section {SectionName} is already initialized in Consul",
                    sectionName);
                return true;
            }
            ...
            _logger.LogInformation(
                "Successfully synced {Count} values from configuration section {SectionName} to Consul",
