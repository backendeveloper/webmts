...
public class KibanaIndexInitializer : BackgroundService
{
        ...
        IConfiguration configuration)
    {
        ...
        _indexPattern = configuration["Elasticsearch:IndexPattern"] ?? "webmts-auth-*";
    ...
    {
        // Kibana'nın başlaması için bir süre bekleyin
    ...
    private async Task CreateIndexPatternAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            // İndeks pattern adını hazırla (yıldız işaretini kaldır)
            string patternId = _indexPattern.Replace("*", "").ToLowerInvariant();
            
            // İndeks pattern zaten var mı?
            ...
            if (createResponse.IsSuccessStatusCode)
            {
                ...
                // Varsayılan indeks olarak ayarla
